...
import { ACCESS_TOKEN, ACCESS_TOKEN_EXPIRE_TIME, JWT_SECRET, REFRESH_TOKEN, REFRESH_TOKEN_EXPIRE_TIME } from '@shared/constants'
...
export const performRefreshToken = async (refreshToken: string | undefined): Promise<CookiePayload | null> => {
...
export const verifyRefreshToken = async (refreshToken: string | undefined) => {
    if (refreshToken) {
        const { userId, token } = jwt.verify(refreshToken, secretOrKey) as RefreshTokenModel
...
export const prepareCookiesPayload = (accessToken: string, refreshToken: string | null | undefined) => {
    const result = { [ACCESS_TOKEN]: accessToken } as CookiePayload
    if (refreshToken) {
        result[REFRESH_TOKEN] = refreshToken
    }
